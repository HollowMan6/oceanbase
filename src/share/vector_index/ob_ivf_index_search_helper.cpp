    ...
    sql::ObSQLSessionInfo *session)
{
  ...
  if (IS_INIT) {
    ...
    LOG_WARN(("failed to get vector " + label_ + "probes").c_str(), K(ret), K_(n_probes));
  } else {
    ...
    qvector_ = qvector;
    conn_ = conn;
    allocator_.set_attr(lib::ObMemAttr(tenant_id, label_ + "Scan"));
    ...
    if (OB_FAIL(schema::ObMultiVersionSchemaService::get_instance().get_tenant_schema_guard(
      ...
      if (n_probes_ == 0) {
        n_probes_ = sqrt(get_vector_lists(index_table_schema));
