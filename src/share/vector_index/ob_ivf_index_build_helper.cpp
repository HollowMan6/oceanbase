...
void ObIvfIndexBuildHelper::destroy() {
  ...
  allocator_.reset();
  arena_allocator_.reset();
  // LOG_INFO("ObIvfIndexBuildHelper destroy finish");
}
...
void ObIvfIndexBuildHelper::destory_vector(ObTypeVector *&vector) {
...
void ObIvfIndexBuildHelper::reuse_array(ObIArray<ObTypeVector *> &array) {
...
int ObIvfIndexBuildHelper::alloc_random_vector(ObTypeVector *&vector,
...
int ObIvfIndexBuildHelper::alloc_and_copy_vector(const ObTypeVector &other,
                                                     ObTypeVector *&vector) {
  int ret = OB_SUCCESS;
  vector = nullptr;
  void *buf = nullptr;
...
int ObIvfIndexBuildHelper::alloc_vector(ObTypeVector *&vector,
                                            const int64_t vector_size) {
  int ret = OB_SUCCESS;
  vector = nullptr;
  void *buf = nullptr;
...
int ObIvfIndexBuildHelper::construct_batch_insert_container_sql_simple(
  ...
  int ret = OB_SUCCESS;
  if (ObIvfBuildStatus::FINISH != status_) {
...
int ObIvfIndexBuildHelper::construct_select_sql_string_simple(
...
int ObIvfIndexBuildHelper::build() {
  ...
  if (OB_ISNULL(cache_)) {
    ...
    switch (status_) {
    case ObIvfBuildStatus::PREPARE_CENTERS: {
    ...
    case ObIvfBuildStatus::INIT_CENTERS: {
    ...
    case ObIvfBuildStatus::RUNNING_KMEANS: {
      if (elkan_kmeans_ && OB_FAIL(ivf_elkan_kmeans())) {
        LOG_WARN("failed to do ivf kmeans", K(ret));
      } else if (!elkan_kmeans_ && OB_FAIL(ivf_kmeans())) {
        LOG_WARN("failed to do ivf kmeans", K(ret));
      }
      break;
    }
    case ObIvfBuildStatus::FINISH: {
      break;
    }
    default: {
      ret = OB_ERR_UNEXPECTED;
...
int ObIvfIndexBuildHelper::init_first_center() {
  int ret = OB_SUCCESS;
  ObTypeVector vector;
  double max_distance = 0;
  double min_distance = DBL_MAX;
  if (ObIvfBuildStatus::PREPARE_CENTERS != status_) {
    ret = OB_STATE_NOT_MATCH;
    LOG_WARN("status not match", K(ret), K_(status));
...
int ObIvfIndexBuildHelper::quick_init_centers() {
  int ret = OB_SUCCESS;
  ObTypeVector vector;
...
// count(*) first to opt
int ObIvfIndexBuildHelper::init_centers() {
  int ret = OB_SUCCESS;
  ObTypeVector vector;
  double max_distance = 0;
  double min_distance = DBL_MAX;
  ...
  int64_t nearest_center_idx = -1;
  if (ObIvfBuildStatus::INIT_CENTERS != status_) {
    ret = OB_STATE_NOT_MATCH;
    LOG_WARN("status not match", K(ret), K_(status));
...
int ObIvfIndexBuildHelper::ivf_kmeans() {
  int ret = OB_SUCCESS;
  ObTypeVector vector;
  int64_t nearest_center_idx = -1;
  double min_distance = DBL_MAX;
  ...
  if (ObIvfBuildStatus::RUNNING_KMEANS != status_) {
    ret = OB_STATE_NOT_MATCH;
    LOG_WARN("status not match", K(ret), K_(status));
  } else if (lists_ != center_vectors_[cur_idx_].count()) {
    ret = OB_ERR_UNEXPECTED;
    ...
    LOG_INFO("success to ivf kmeans", K(ret), KPC(this));
  }
  return ret;
}

//// elkan kmeans
int ObIvfIndexBuildHelper::get_vector_by_sql(const int64_t offset,
...
int ObIvfIndexBuildHelper::init_first_center_elkan() {
  int ret = OB_SUCCESS;
  ObTypeVector vector;
  if (ObIvfBuildStatus::PREPARE_CENTERS != status_) {
    ret = OB_STATE_NOT_MATCH;
...
// kmeans++
int ObIvfIndexBuildHelper::init_centers_elkan() {
  int ret = OB_SUCCESS;
  ObTypeVector vector;
  double distance = 0;
  ObTypeVector *next_center = nullptr;
...
int ObIvfIndexBuildHelper::ivf_elkan_kmeans() {
  int ret = OB_SUCCESS;
  ObTypeVector vector;
  int64_t nearest_center_idx = -1;
  double min_distance = DBL_MAX;
...
int ObIvfIndexBuildHelper::get_nearest_center(const ObTypeVector &vector,
